...
return (
    ...
    <section className="op-container-bf menu_bf">
        ...
        return (
            <button className= "add-bf" key={product.id} value={product.name} onClick={()=>{setOrder([...order, {name:product.name, price:product.price, id:product.id}]);
            onAdd(product)
            console.log(order)}}> 
                {product.name} <br/> ${product.price}
            </button>
        )})}                   
    <Comanda reset={resetElem} onAdd={onAdd} onRemove={onRemove} order={order} client={client} table={table} category={category} addOrder={addOrder}/>
    </section>
    </div>
